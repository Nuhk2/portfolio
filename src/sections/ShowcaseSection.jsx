...
const projects = [
  ...
  },
  {
    ...
    uiDesign: "https://www.behance.net/gallery/237435805/Quetta-Caf-Website-Design-Brand-Experience", // replace with real link
...
const NextArrow = ({ onClick }) => (
  <button
    onClick={onClick}
    className="absolute right-0 top-1/2 -translate-y-1/2 z-20 rounded-full p-3 transition"
    style={{
...
const PrevArrow = ({ onClick }) => (
  <button
    onClick={onClick}
    className="absolute left-0 top-1/2 -translate-y-1/2 z-20 rounded-full p-3 transition"
    style={{
      backgroundColor: 'var(--bg-card)',
      color: 'var(--text-primary)',
    }}
  ...
  if (gitLink && gitLink.trim() !== "") {
    buttons.push(
      <a
        ...
        className="px-5 py-2 rounded-xl font-small transition-all duration-500"
        style={{
          backgroundColor: 'var(--bg-card)',
...
<div className="w-full h-full md:px-20 px-5">
  ...
  return (
    <div key={index} className="px-4 pt-4 pb-16">
      ...
      >
        <h2 className="text-2xl font-semibold" style={{ color: 'var(--text-primary)' }}>{proj.title}</h2>
        <p className="md:text-lg mt-2" style={{ color: 'var(--text-secondary)' }}>{proj.short}</p>
  ...
  <p className="text-sm leading-tight transition-opacity duration-500" style={{ color: 'var(--text-secondary)' }}>
